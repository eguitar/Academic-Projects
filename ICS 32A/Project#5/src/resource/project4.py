...
class ColumnsGame:
        ...
        while self._running:
            if arcade._matching:
                try:
                    ...
                    arcade.drop_cells()
                    arcade.find_match()                    
                except game.GameOverOutOfBound:
                    self._print_board(arcade)
                    print('GAME OVER')
                    break
            else:
                try:
                    ...
                    if arcade._matching:
                        try:
                            arcade.find_match()
                        except columns.GameOverOutOfBound:
                            self._print_board(arcade)
                            print('GAME OVER')
                            break
                        except columns.GameOverInvalidFaller:
                            print('GAME OVER')
                            break
    ...
    def _create_board(self) -> game.Board:
        ...
        row = int(input())
        col = int(input())
        arcade = columns.Board((col,row))
        return arcade

    def _setting_field(self, arcade: columns.Board) -> None:
        mode = input()
        if mode == 'CONTENTS':
            board_field = []
    ...
    def _print_board(self,arcade: columns.Board):
    ...
    def _handles_events(self, arcade: columns.Board, user: str):
