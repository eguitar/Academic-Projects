...
# -------------------------------------------------------------
    def create_faller(self, col: int, colors: list):
        ...
        if not self._faller_exists():
            self._faller = Faller(col-1, colors)
    ...
    def rotate_faller(self):
        ...
        if not self._faller_exists(): pass
        elif self._faller._colors == []:
            x = self._faller._rows[2]
            index = self._board[self._faller._col][x]
            for i in reversed(range(1,len(self._faller._rows))):
                f_row = self._faller._rows[i]
                i_row = self._faller._rows[i-1]
                self._board[self._faller._col][f_row] = self._board[self._faller._col][i_row]
            row = self._faller._rows[0]
            self._board[self._faller._col][row] = index
        elif len(self._faller._colors) == 2:
            self._faller._colors.insert(0,self._board[self._faller._col][0])
            self._board[self._faller._col][0] = self._faller._colors.pop()
        else:
            self._faller._colors.insert(0,self._board[self._faller._col][1])
            self._board[self._faller._col][1] = self._board[self._faller._col][0]
            self._board[self._faller._col][0] = self._faller._colors.pop()
    ...
    def fall_faller(self):
        ...
        if self._faller_exists():
            ...
            index = None
            counter = 0
            if self._status[self._faller._col][0] == 1:
                raise GameOver
            ...
            for row in reversed(range(1,index)):
                if self._valid_cell(self._faller._col,row) and counter != 3:
                    self._board[self._faller._col][row] = self._board[self._faller._col][row-1]
                    self._status[self._faller._col][row] = self._status[self._faller._col][row-1]
                    rows.insert(0,row)
                    self._board[self._faller._col][row-1] = ' '
                    self._status[self._faller._col][row-1] = 0
                    counter += 1
            if self._faller._colors != [] \
               and self._valid_cell(self._faller._col,0):
                    self._board[self._faller._col][0] = self._faller._colors.pop()
                    self._status[self._faller._col][0] = 2
                    if len(self._faller._colors) != 3:
                        rows.insert(0,0)
            if self._faller._falling:
                self._faller._rows = rows
# -------------------------------------------------------------
            ...
            while running:
                ...
                for row in reversed(range(self._row)):
                    if self._status[col][row] == 0:
                        index = row + 1
                        break
                if index == None and not self._faller_exists(): break
                elif index == None and self._faller_exists(): raise GameOver 
                ...
                if self._faller_exists() \
                   and self._faller._col == col and self._faller._colors != []:
                    self._board[col][0],self._status[col][0]=self._faller._colors.pop(),1
                running = False
                index = None
                for row in reversed(range(self._row)):
                    if self._status[col][row] == 0:
    ...
    def find_match(self):
        ...
        if matched_points != []:
            ...
            if not self._faller_exists() or self._faller._colors == []:
                self._matching = False
                self.reset_faller()
...
# -------------------------------------------------------------
    ...
    def _is_faller_land(self) -> bool:
        ...
        if self._faller_exists():
            ...
            if index == self._row-1 or self._status[self._faller._col][index+1] == 1:
                return True
            else: return False
        else: return False
...
# -------------------------------------------------------------
    ...
    def _faller_status(self) -> int:
        ...
        the faller'''
        return self._status[self._faller._col][self._faller._rows[0]]
