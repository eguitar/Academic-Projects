...
%~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
fncs={'forwardNumericalDeriv','backwardNumericalDeriv','centralNumericalDeriv','trapezoidInt','simpsonInt'};
firstCodingAssngm = 6;
testOrGrade = 'test';
%~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
%~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
%~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
        if currentFunc == 1
            clear x;
            x=transpose(linspace(-pi,pi,10000));
            if max(abs(cos(x(1:(end-1)))-feval(string(append(fncs(currentFunc),'_',stdID)),@sin,x)))<5*10^(-4)
        ...
        if currentFunc == 2
            clear x;
            x=transpose(linspace(-pi,pi,10000));
            if max(abs(cos(x(2:end))-feval(string(append(fncs(currentFunc),'_',stdID)),@sin,x)))<5*10^(-4)
                results(currentStudent,currentFunc+1)=1;
            else
                results(currentStudent,currentFunc+1)=0;
            end
        ...
        if currentFunc == 3           
            clear x;
            x=transpose(linspace(-pi,pi,10000));
            if max(abs(cos(x(2:(end-1)))-feval(string(append(fncs(currentFunc),'_',stdID)),@sin,x)))<5*10^(-4)
                results(currentStudent,currentFunc+1)=1;
            else
                results(currentStudent,currentFunc+1)=0;
            end
        ...
        if currentFunc == 4
            if abs(feval(string(append(fncs(currentFunc),'_',stdID)),@(x)(x^2),0,1,10)-0.335)<10^(-4)
                results(currentStudent,currentFunc+1)=1;
            else
                results(currentStudent,currentFunc+1)=0;
            end
        ...
        if currentFunc == 5
            if abs(feval(string(append(fncs(currentFunc),'_',stdID)),@(x)(x^2),0,1,3)-0.259259259259259)<10^(-4)
                results(currentStudent,currentFunc+1)=1;
            else
                results(currentStudent,currentFunc+1)=0;
            end
