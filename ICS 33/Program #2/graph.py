...
class Graph:
    ...
    def __init__(self,legal_edge_value_predicate,*edges):
        ...
        for e in edges:
            if type(e) == str:
                if e in self.edges:
                    raise GraphError('Node is already in graph')
                self.edges[e] = {}
            elif type(e) == tuple and len(e) == 3:
                if self.is_legal_edge_value(e[2]):
                    if e[0] in self.edges and e[1] in self.edges[e[0]]:
                        raise GraphError('Node pair already in graph')
                ...
                raise GraphError('Argument is not a str or tuple')
