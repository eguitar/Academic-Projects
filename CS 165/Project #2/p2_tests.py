...
# Instructions
# Some test cases for the ZipTree and bin packing algorithms can be found in the main block below.
#
...
def is_equal(v1: [float], v2: [float]) -> bool:
...
def test_algorithm(_test: ProblemInstance, expected_result: ProblemInstance, algo: 'algorithm', name: str):
	test = deepcopy(_test)
	algo(test.items, test.assignments, test.free_space)

	if test.assignments == expected_result.assignments and is_equal(test.free_space, expected_result.free_space):
