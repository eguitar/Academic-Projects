...
class ZipTree:
		...
		while curr_node != None:
			...
			if curr_node.key == end_key:
				if curr_node.left != None:  ## need to test
					self.__update_best_brc(curr_node.left)
				if curr_node.right != None:
					self.__update_best_brc(curr_node.right)
				break
			elif key < curr_node.key:
				curr_node = curr_node.left
			else:
	...
	def __update_best_brc(self, node: Node):
		if node == None:
			return
		node.brc = self.__get_brc1(node)
	...
	def __update_first_brc(self, node: Node):
		if node == None:
			return
		node.brc = self.__get_brc2(node)
