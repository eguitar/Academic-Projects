...
TEST(HashFunctionTest, Hf6)
{
	...
	unsigned shouldBe = 6*1+5*100+4*100*100+3*100*100*100+2*100*100*100*100+1*100*100*100*100*100;
	shouldBe = shouldBe % 10;
	EXPECT_EQ(hv, shouldBe);
}
...
TEST(HashFunctionTest, Hf7)
{
	...
	unsigned shouldBe = 1+2*100+3*100*100+4*100*100*100+5*100*100*100*100+6*100*100*100*100*100;
	shouldBe = shouldBe % 10;
	EXPECT_EQ(hv, shouldBe);
}
